...
// filepath: /workspaces/zapper/tests/index.test.js
...
describe("Authentication", () => {
  test("User is able to sign up only once", async () => {
    const username = "deviant" + Math.random(); // deviant0.12331313
  ...
  test("Signup request fails if the username is empty", async () => {
    const username = `deviant-${Math.random()}`; // deviant-0.12312313
    const password = "12345678";
  ...
  test("Signin succeeds if the username and password are correct", async () => {
    const username = `deviant-${Math.random()}`;
    const password = "12345678";
...
describe("User metadata endpoint", () => {
  ...
  beforeAll(async () => {
    ...
    console.log("avatar response is " + avatarResponse.data.id);
...
describe("Arena endpoints", () => {
  ...
  test("Delete endpoint is able to delete an element", async () => {
    ...
    // Extract the SpaceElement ID to delete
    const spaceElementId = response.data.elements[0].id;
    console.log("SpaceElement to be deleted", response.data.elements[0]);

    console.log("Attempting to delete element with ID:", spaceElementId);
    ...
    expect(deleteResponse.data.message).toBe("Element deleted!");

    // Fetch the space details after deletion
    ...
    // Assert that the number of elements has decreased by one
...
describe("Websocket tests", () => {
  ...
  async function setupHTTP() {
    ...
    adminToken = adminSignInResponse.data.taken;
  ...
  test("If a user leaves, the other users recieves a leave event", async () => {
    ws1.close();
    const message = await waitForAndPopLatestMessage(ws2Messages);
